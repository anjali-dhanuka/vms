...
def calculate_duration(start_time, end_time):
    ...
    working_hours = float((end_delta - start_delta).seconds) / 3600
...
def get_report_by_id(report_id):
    """
    fetches report object by id

    :param report_id: The id of report
...
def generate_report(volunteer_shift_list):
    ...
    for volunteer_shift in volunteer_shift_list:
        ...
        report["duration"] = calculate_duration(volunteer_shift.start_time,
...
def get_volunteers_by_shift_id(s_id):
    ...
    # order by name
    volunteer_list = volunteer_list.order_by('first_name', 'last_name')
